...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "vehicle detection task using yolov5s(custom model), using OpenVINO(AUTO) runtime.";
    ...
    while (cap.isOpened()) {
        ...
        if (results[0].show(
            false, 0.5f, DrawParam(), // show annotated image & input image(640*384) with unblock mode
