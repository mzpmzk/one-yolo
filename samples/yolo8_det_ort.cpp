...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "obstacle/unclear detection task using yolov8s(custom model), using ONNXRuntime(CPU)";
    cfg.version     = YoloVersion::YOLO8;
    cfg.task        = YoloTaskType::DET;
    cfg.target_rt   = YoloTargetRT::ORT_CPU;
    cfg.model_path  = "./vp_data/models/det_cls/landslide_yolov8s-c3.onnx";
    cfg.input_w     = 640;
    cfg.input_h     = 384;
    cfg.batch_size  = 1;
    cfg.num_classes = 3;
    cfg.names       = {"landslide", "obstacle", "roadblock"};
    ...
    cv::VideoCapture cap("./vp_data/test_video/unclear.mp4");
    while (cap.isOpened()) {
        // collect frame
        cv::Mat frame;
        if (!cap.read(frame)) {
