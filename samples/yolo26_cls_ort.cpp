...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "imagenet classification task using yolov26n(model from ultralytics/ultralytics repo), using ONNXRuntime(CPU) runtime.";
    cfg.version     = YoloVersion::YOLO26;
    cfg.task        = YoloTaskType::CLS;
    cfg.target_rt   = YoloTargetRT::ORT_CPU;
    cfg.model_path  = "./vp_data/models/ultralytics/yolo26n-cls.onnx";
    cfg.input_w     = 224;
    cfg.input_h     = 224;
    cfg.batch_size  = 1;
    ...
    */
    auto results = model(std::vector<cv::Mat>{image1});
