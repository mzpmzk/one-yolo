...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "coco detection task using yolov8n(model from ultralytics/ultralytics repo), using default(OpenCV::DNN/CUDA) runtime.";
    cfg.version     = YoloVersion::YOLO8;
    cfg.task        = YoloTaskType::DET;
    cfg.target_rt   = YoloTargetRT::OPENCV_CUDA;
    cfg.model_path  = "./vp_data/models/ultralytics/yolov8n.onnx";
    cfg.input_w     = 640;
    cfg.input_h     = 640;
    cfg.batch_size  = 1;
    cfg.num_classes = 80;
    ...
    while (cap.isOpened()) {
        ...
        DrawParam param;
        param.tracks = false;        // do not draw tracks
        if (results[0].show(
