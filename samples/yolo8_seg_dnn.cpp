...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "coco segmentation task using yolov8n(model from ultralytics/ultralytics repo), using default(OpenCV::DNN/CUDA) runtime.";
    ...
    while (cap.isOpened()) {
        ...
        DrawParam param;             // define draw parameter
        //param.mask_alpha = 0;        // transparent for mask
        //param.mask_line_width = 3;   // bold for contour of mask
        if (results[0].show(
            false, 1.0f, param,      // show annotated image & input image(640*640) with unblock mode
