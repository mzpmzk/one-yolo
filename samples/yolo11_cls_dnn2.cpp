...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "imagenet classification task using yolov11n(model from ultralytics/ultralytics repo), get structured results separately, using default(OpenCV::DNN/CUDA) runtime.";
    ...
    */
    ...
    /* 5. get structured results separately */
    ...
    /* 6. print/save/send results(as string format) via kafka/udp/... to 3rd-party system */
    auto str_top1          = std::to_string(top1);            // 569
    auto str_top_conf      = yolo::to_string(top1_conf);      // 0.34
    auto str_top1_label    =                 top1_label;      // garbage truck
    auto str_top5          = yolo::to_string(top5);           // [569,867,847,586,408]  
    auto str_top5_confs    = yolo::to_string(top5_confs);     // [0.3355742394924164,0.28039076924324036,0.16061078011989594,0.030564509332180023,0.02643916942179203]
    auto str_top5_labels   = yolo::to_string(top5_labels);    // ["garbage truck","semi-trailer truck","tank","half-track","amphibious vehicle"]
}