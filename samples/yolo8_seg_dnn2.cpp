...
int main() {
    ...
    YoloConfig cfg;
    cfg.desc        = "coco segmentation task using yolov8n(model from ultralytics/ultralytics repo), draw annotated image by myself, using default(OpenCV::DNN/CUDA) runtime.";
    cfg.version     = YoloVersion::YOLO8;
    cfg.task        = YoloTaskType::SEG;
    cfg.target_rt   = YoloTargetRT::OPENCV_CUDA;
    cfg.model_path  = "./vp_data/models/ultralytics/yolov8n-seg.onnx";
    cfg.input_w     = 640;
    cfg.input_h     = 640;
    cfg.batch_size  = 1;
    cfg.approx_f    = 0.0005f;
    cfg.num_classes = 80;
    ...
    cv::VideoCapture cap("./vp_data/test_video/mask_rcnn.mp4");
    while (cap.isOpened()) {
        // collect frame
        cv::Mat frame;
        if (!cap.read(frame)) {
        ...
        // draw & show annotated image by myself instead of YoloResult::show()
        ...
        /* 5-1. set original image as canvas */
        auto& canvas = frame;

        /* 5-2. draw all tracks */
        for (size_t i = 0; i < track_points.size(); i++) {
            auto& one_track = track_points[i];
            cv::polylines(canvas, one_track, false, cv::Scalar(255, 0, 255), 2);
        }
        
        /* 5-3. draw all contours */
        cv::drawContours(canvas, contours, -1, cv::Scalar(0, 0, 255), 2);

        /* 5-4. draw all boxes & labels */
        for (size_t i = 0; i < boxes.size(); i++) {
            cv::rectangle(canvas, boxes[i], cv::Scalar(255, 0, 0), 2);
            cv::putText(canvas, labels[i], cv::Point(boxes[i].x, boxes[i].y), 1, 1, cv::Scalar(255, 255, 0));
        }

        /* 5-5. show annotated image */
